...
| iSCSI/multipath | Yes | Stable logical storage, add/replace/remove path lifecycle, health normalization, active path-group identity and durable failover transitions | Yes | Ubuntu runs `32589493527` and `32589920257` passed a real LIO/open-iscsi/DM-Multipath lifecycle across three loopback portals, including continuous verified I/O during both path failures, multipathd restart and bounded busy-map recovery | Not tested | `targetcli-fb`, `open-iscsi`, multipath; isolated loopback evidence is not physical array certification and polling observations are not counted as failovers |
