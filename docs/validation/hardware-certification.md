...
| iSCSI/multipath | Yes | Stable logical storage, add/replace/remove path lifecycle, health normalization, active path-group identity and durable failover transitions | Yes | Ubuntu run `32589493527` passed a real LIO/open-iscsi/DM-Multipath lifecycle across three loopback portals, including continuous verified I/O during both path failures and multipathd restart | Not tested | `targetcli-fb`, `open-iscsi`, multipath; isolated loopback evidence is not physical array certification and polling observations are not counted as failovers |
