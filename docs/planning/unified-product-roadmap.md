...
| ID | Priority | Task | Acceptance | Status |
|---|---|---|---|---|
| IMPORT-01 | P2 | Foreign signature detection | Detect ext4, XFS, Btrfs, NTFS, exFAT, MD, LVM, ZFS, and recognizable NAS layouts. | IN PROGRESS — authenticated assessment recognizes persisted ext4/XFS/Btrfs/NTFS/exFAT and MD/LVM/ZFS member signatures and groups stable member UUIDs; reviewed NAS-origin adapters remain pending and weak filesystem heuristics intentionally report origin as Not reported |
...
| IMPORT-07 | P2 | Synology, QNAP, and generic NAS profiles | Add evidence-based import adapters without manufacturing origin from weak heuristics. | SOFTWARE VERIFIED / HOSTED REGRESSION PENDING — generic Linux MD/LVM/ZFS stacks retain their real provider identity without a vendor guess; a bounded read-only source exporter, strict evidence schema, audited API and Storage UI identify Synology DSM, QNAP QTS/QuTS, or Generic Linux NAS only when every candidate member matches the source runtime export by stable identity; partial, ambiguous, duplicate, marker-conflicting, and Unraid-conflicting evidence fails closed to Not reported |
...
| BACKUP-08 | P2 | Encrypted secrets and fresh-appliance restore | Exclude secrets by default; optionally encrypt; restore and reconcile disks by stable identity. | IN PROGRESS — the default archive excludes secret-like files, environment secret keys, password verifiers/live sessions/tokens, encrypted credential blobs, symlinks and secret-store keys while preserving disabled re-entry configuration; root-only offline fresh-appliance apply now verifies an independent digest, refuses active services/non-fresh state, switches atomically and retains rollback state. Optional encrypted-secret export and automated post-restore disk reconciliation remain |
| BACKUP-09 | P2 | Backup UI/API/E2E | Provide target/setup/history/restore surfaces and test outage, resume, corruption, insufficient space, and fresh restore. | IN PROGRESS — Settings target/setup/test/schedule/run/history/validation UI, Activity linkage, component tests and browser E2E pass; transient outage/resume, corruption and pre-mutation insufficient-space handling pass deterministic tests. Live-MinIO fresh-root apply is in CI; full installed-appliance recovery acceptance remains |
...
| AUTO-06 | P2 | Alert deduplication, routing, and suppression | Add windows, webhook/AppRise-compatible routing, runbook links, and flapping control. | IN PROGRESS — durable rule/entity deduplication, hysteresis, sustained windows, path-flap detection, bounded operator suppression, signed generic-webhook routing and evidence-aware runbooks are implemented; an AppRise-specific adapter remains |
...
| VALID-03 | P2 | Foreign/archive integration | Execute read-only foreign and Unraid-like fixtures plus archive copy/verification reports. | IN PROGRESS — Ubuntu run `32674912616` executed a real loop-backed ext4 read-only inspection through the production executor with persisted evidence and no remaining mount; Unraid-like and archive copy/verification execution remain pending |
