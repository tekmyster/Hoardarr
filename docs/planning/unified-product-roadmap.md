...
| LIFE-14 | P1 | Provider-aware failed-disk replacement | Rebuild a failed member without changing the Storage Group namespace, using provider-specific recovery rather than a generic format action. | IN PROGRESS — SnapRAID data-member production UI/API/executor binds the config and stable replacement identity, displays existing-data evidence, requires exact consent, uses status/fix/audit-check/sync with durable phases, and preserves the namespace; hosted Ubuntu run `32663073443` reconstructed and independently hashed a file on a purpose-created replacement loop, proved persistent `-part1` alias handling, audit-before-sync order, current parity, and a succeeded journal; ZFS and MD replacement remain provider-specific dependencies |
...
| EXPAND-08 | P1 | Expansion tests | Cover mixed sizes, health blockers, parity needs, ZFS restrictions, stale discovery, and unsupported tools. | IN PROGRESS — existing-data/system exclusion, targeting, stale assessment, mergerFS, SnapRAID, tier, ZFS candidate/parser/policy/executor, reserve/release, API, component, and browser cases pass; hosted Ubuntu runs `32660386715` and `32661927103` prove SnapRAID data/parity and matching-vdev ZFS expansion respectively; the broader disposable-Linux fault matrix remains active |
...
| HW-09 | P1 | Live topology UI | Productize real discovered chassis/controller/path/enclosure/bay/disk relationships and contextual actions. | IN PROGRESS — real controller/port/PHY/expander/path/enclosure/bay/disk/logical relationships, honest missing data, rate rails and bay confidence are implemented; contextual lifecycle actions remain HW-11 |
...
| HW-11 | P1 | Contextual hardware actions | Connect Details, SMART, Locate, Test, Drain, Replace, and Decommission only to implemented backend behavior. | IN PROGRESS — topology drive details now route unassigned media into real health-test, import, and setup workflows and route managed media to Storage Group lifecycle; unsupported Locate/generic Replace controls remain intentionally absent pending provider/backend implementation |
...
| MEDIA-06 | P1 | ARR active-operation awareness | Detect downloads, imports, renames, moves, and hardlink-sensitive work through product-aware adapters. | IN PROGRESS — Sonarr/Radarr/Lidarr/Readarr/Whisparr queue adapters now classify bounded downloading, importing, pending and stalled activity without persisting media titles; the durable worker refreshes activity independently of API/browser clients and fresh active writes block drain preflight/execution; explicit rename/move command-state collection remains |
...
| MEDIA-11 | P1 | Tier occupancy and migration UX | Show real configured membership, capacity, queued moves, seeding retention, drain estimates, and failures. | IN PROGRESS — configured membership, capacity/occupancy telemetry, exact transfer review, durable Activity state, failures, seeding retention, and explicit cleanup are productized; aggregate queued-migration drain-time estimation still requires real queued byte totals rather than fabricated estimates |
