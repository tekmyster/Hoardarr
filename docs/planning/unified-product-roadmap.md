...
| LIFE-12 | P1 | Lifecycle end-to-end validation | Prove add, assign, activate, prefer, drain, verify, read-only, retire, and optional reuse/wipe behavior. | SOFTWARE VERIFIED — real browser covers preferred-write through durable drain/pause/resume/report/retirement and explicit release for reuse; two-loop ext4 covers actual movement and restart; wipe remains intentionally independent and already capability-gated |
...
| DRAIN-12 | P1 | Drain failure matrix | Test full destination, disappearing source/destination, ARR conflict, checksum failure, pause/resume, and worker crash. | IN PROGRESS — full/capacity, identity, collision, ARR conflict, pause/resume, and worker restart are covered; disappearing-root and BLAKE3 mismatch fail-closed regressions added for the next Linux run |
