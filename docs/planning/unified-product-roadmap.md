...
| IMPORT-04 | P2 | Unraid disk and parity classification | Recognize independent data disks and distinguish identified, suspected, and unknown parity. | SOFTWARE VERIFIED — a bounded read-only Unraid runtime-state exporter and persisted assignment-provenance model match exact stable serial/WWN identity, expose authenticated/audited load/replace/forget APIs and a real Storage UI, reject role/slot/identity conflicts, identify assignment-backed data/parity, keep compatible standalone filesystems only suspected data, keep filesystem-free capacity evidence only suspected parity, and never claim parity validity or reuse; disposable Ubuntu loop execution is wired into the extended storage workflow pending its next hosted run |
...
| IMPORT-12 | P2 | Foreign/import integration tests | Exercise disposable filesystems, Unraid-like disks, malformed layouts, collisions, read errors, and restart recovery. | IN PROGRESS — deterministic executor tests cover signature drift, unexpected read-write state, guaranteed detach, detach failure, bounded inventory, symlink exclusion, API authorization/idempotency and durable worker completion; a real disposable ext4 loop workflow runs the production mount/probe/inventory/unmount path, and a two-loop Unraid-like data/parity classification scenario is wired into hosted Ubuntu; injected read errors, migration collisions, and restart recovery remain pending |
