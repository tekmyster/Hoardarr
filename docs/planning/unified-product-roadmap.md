...
| LIFE-13 | P1 | Guided disk onboarding lifecycle | Turn detected, unassigned drives into a plain-language assign/activate/prefer workflow while preserving the Advanced plan and identity evidence. | SOFTWARE VERIFIED — browser E2E follows a detected data-bearing ext4 disk through honest used/foreign classification, Import existing data, preserve-data immutable review, no partition/filesystem creation, no destructive consent, durable apply, refresh/recovery, and the stable `/data/media` namespace; loading/error/Activity behavior and backend invariants remain covered |
| LIFE-14 | P1 | Provider-aware failed-disk replacement | Rebuild a failed member without changing the Storage Group namespace, using provider-specific recovery rather than a generic format action. | VERIFIED IN ISOLATION — SnapRAID, ZFS, and Linux MD share authenticated immutable review, exact destructive consent, stable replacement identity, live provider identity/topology revalidation, durable Activity phases, existing-data evidence, and a real Storage UI. Browser E2E now proves degraded Linux MD RAID5 replacement review binds the exact array UUID, data-bearing replacement warning, `I AGREE`, durable progress and succeeded result; hosted Ubuntu loop runs preserve hashes and pool/array identity. Physical-vendor certification remains separate |
...
| EXPAND-04 | P1 | ZFS, new-pool, cache, reserve, and Linux MD candidates | Generate supported vdev/pool/tier/reserve/array choices without inventing executable capability. | VERIFIED IN ISOLATION — matched mirror/RAIDZ1/2/3 pools, download tiers, reserve/release, matching-vdev ZFS expansion, and exact new Linux MD RAID1/5/6/10 candidates are implemented. MD capacity uses matched member capacity and level-specific formulas, odd RAID10 is rejected, and no generic MD reshape is promised. Hosted Ubuntu ZFS execution preserved the pool GUID; unavailable or ambiguous capabilities remain fail-closed |
...
| EXPAND-06 | P1 | Guided and Advanced expansion wizard | Recommend in plain language while exposing exact geometry and policies on demand. | SOFTWARE VERIFIED — browser E2E proves snapshot-bound mergerFS, RAIDZ2, and Linux MD RAID10 choices open the real wizard with the complete disk set and exact selected geometry; existing ZFS expansion exposes the exact pool, mount, new members, and immutable matching geometry |
| EXPAND-07 | P1 | Immutable expansion apply | Bind approval to hardware and plan, revalidate identity, and execute through durable operations. | SOFTWARE VERIFIED — every candidate carries candidate/disk/target/configuration/snapshot bindings; browser E2E proves exact candidate selection survives review and durable mergerFS reconciliation, while Linux MD plans reject altered level/member count and existing ZFS revalidates live GUID and canonical topology before irreversible execution |
...
| ID | Priority | Task | Acceptance | Status |
|---|---|---|---|---|
| HW-01 | P1 | SMART short/long orchestration | Detect support, start tests, track ETA/state/result, and represent passthrough limitations honestly. | SOFTWARE VERIFIED — bounded `smartctl -j -c` capability detection exposes supported/unsupported/not-reported plus drive-reported durations. Storage provides exact Run Short Test and Run Long Test controls; browser E2E proves short/extended immutable plans, non-destructive consent behavior, drive-reported duration/source and durable Activity history. The live bench correctly disables both because passthrough support is Not reported; physical execution remains HW-16 |
...
| HW-03 | P1 | Full physical topology model | Represent PCI BDF → HBA/controller → SAS host/PHY → expander → enclosure/bay → disk → path/logical storage/group. | SOFTWARE VERIFIED — the normalized graph preserves PCI BDF controller identity separately from the SAS host and connects port, PHY, expander, path, enclosure, confirmed/inferred/unknown bay, stable drive, filesystem, pool/share and Storage Group. A browser regression renders the complete chain; physical execution remains HW-16 |
...
| VALID-05 | P1 | Topology fixture regression | Parse sanitized real SCSI/SAS evidence and assert bay confidence, slow links, path loss, and drift. | SOFTWARE VERIFIED — sanitized SAS2308/SAS3008 and DS424IOM6/DS224IOM6 fixtures exercise confirmed/inferred/unknown bay evidence, 3/6/12 Gb/s interpretation, PHY errors, path loss, enclosure disappearance and drift without embedding real identifiers; browser E2E asserts and screenshots the rendered chain. Matching physical-provider certification remains HW-16 |
| VALID-06 | P1 | Two-pass release gates | Run focused and clean regression passes; update traceability without conflating fixture, VM, and physical evidence. | SOFTWARE VERIFIED — immutable commit `66246ac274479724cebc61297b595935fbc1e012` passed CI run `32718177066`: 542 backend, 162 frontend, 4 accessibility, 33 Chromium E2E, 74 bootstrap and 19 release tests plus Ruff, bounded telemetry soak, Linux sources, wheel/release/systemd, installed appliance and live MinIO. Storage run `32718177058` passed all four applicable isolated jobs. Archive digest `7e764ecb5af30dc9947df40b19f12839df5ea1cd7f134e9f5424e8820dd27536` was independently verified and deployed to the beta bench |
