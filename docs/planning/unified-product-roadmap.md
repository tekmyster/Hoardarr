...
| LIFE-12 | P1 | Lifecycle end-to-end validation | Prove add, assign, activate, prefer, drain, verify, read-only, retire, and optional reuse/wipe behavior. | IN PROGRESS — real browser now covers activation review for both exact-mounted backends, preferred-write, durable drain/pause/resume/report/retirement, and explicit release; the two-loop ext4 harness now enters lifecycle through the same immutable activation proof, with CI re-execution pending for this changed Linux boundary; wipe remains intentionally independent and capability-gated |
...
| EXPAND-03 | P1 | mergerFS and SnapRAID candidates | Generate add-member, data-disk, and parity-expansion candidates with restrictions. | SOFTWARE VERIFIED — bounded SnapRAID configuration discovery maps an exact config to mergerFS branches; separate data and capacity-qualified parity candidates bind the config digest and explicit role, preserve zero usable-capacity semantics for parity, revalidate before structured status/sync execution, roll back pre-sync failures, and retain the visible member/config in needs-attention state after an incomplete sync; disposable Linux execution remains EXPAND-08 |
| EXPAND-04 | P1 | ZFS, new-pool, cache, and reserve candidates | Generate supported vdev/pool/tier/reserve choices without inventing executable capability. | IN PROGRESS — matched mirror, RAIDZ1/2/3 single-vdev, new-pool, download-tier, and authenticated/audited reserve/release candidates are software-verified; multi-vdev expansion recommendations remain pending |
...
| EXPAND-06 | P1 | Guided and Advanced expansion wizard | Recommend in plain language while exposing exact geometry and policies on demand. | SOFTWARE VERIFIED — browser E2E proves snapshot-bound mergerFS and RAIDZ2 recommendations open the real wizard with the complete disk set and exact selected geometry; remaining multi-vdev choices track under EXPAND-04 |
| EXPAND-07 | P1 | Immutable expansion apply | Bind approval to hardware and plan, revalidate identity, and execute through durable operations. | SOFTWARE VERIFIED — every candidate now carries a normalized configuration as well as candidate ID, disk set, optional exact target, and assessment digest into the canonical plan; policy validation rejects changed topology/vdev geometry and the executor retains live identity revalidation |
| EXPAND-08 | P1 | Expansion tests | Cover mixed sizes, health blockers, parity needs, ZFS restrictions, stale discovery, and unsupported tools. | IN PROGRESS — existing-data, system-disk exclusion, matched-instance targeting, stale assessment rejection, mergerFS runtime/persistence, SnapRAID, download-tier, matched mirror/RAIDZ math and exact-plan binding, reservation persistence/system protection, API, component, and real-browser tests added; disposable-Linux fault matrix pending |
