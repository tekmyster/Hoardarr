...
| IMPORT-03 | P2 | MD/LVM/ZFS foreign assembly preview | Identify members and mountability without activating or importing automatically. | SOFTWARE VERIFIED — immutable snapshot/member/signature-bound reviews use structured `mdadm --examine --export`, LVM `--readonly --foreign --devices`, or offline `zdb -l` probes; MD/LVM completeness is reported only from provider evidence, ZFS mountability and inactive-stack health remain honestly Not reported, activation/import commands are absent, the authenticated API is scoped/audited, and the real Storage UI/browser flow exposes no activation control. Disposable Ubuntu tool execution is implemented in `foreign_stack_preview.py` and pending the next isolated CI result. |
...
| IMPORT-12 | P2 | Foreign/import integration tests | Exercise disposable filesystems, Unraid-like disks, malformed layouts, collisions, read errors, and restart recovery. | IN PROGRESS — deterministic executor tests cover signature drift, unexpected read-write state, guaranteed detach, detach failure, bounded inventory, symlink exclusion, API authorization/idempotency and durable worker completion; a real disposable ext4 loop workflow now runs the production mount/probe/inventory/unmount path, while Unraid-like layouts, injected read errors, and restart recovery remain pending |
