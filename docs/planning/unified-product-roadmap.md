...
| VALID-03 | P2 | Foreign/archive integration | Execute read-only foreign and Unraid-like fixtures plus archive copy/verification reports. | IN PROGRESS — isolated CI now contains a real loop-backed ext4 read-only inspection with persisted evidence and no remaining mount; Unraid-like and archive copy/verification execution remain pending |
