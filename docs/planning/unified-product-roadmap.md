...
| LIFE-06 | P1 | Drain, verify, read-only, and retire states | Model DRAIN → VERIFY → READ-ONLY → RETIRE without permitting unsafe direct skips. | IMPLEMENTED + SOFTWARE VERIFIED — durable operation owns each guarded transition; Linux execution pending |
...
| LIFE-12 | P1 | Lifecycle end-to-end validation | Prove add, assign, activate, prefer, drain, verify, read-only, retire, and optional reuse/wipe behavior. | IN PROGRESS — disposable Linux drain job added; remote execution result pending; optional reuse/wipe remains separate |
...
| DRAIN-04 | P1 | Checkpointed mover | Copy through durable checkpoints and temporary destinations without deleting the verified source. | IMPLEMENTED + SOFTWARE VERIFIED — descriptor-relative no-follow copy, atomic no-replace publish, durable per-file checkpoints; Linux execution pending |
...
| DRAIN-06 | P1 | ARR active-write awareness and windows | Coordinate imports/downloads/renames and configurable maintenance windows. | IN PROGRESS — ARR write-sensitive preflight and safe-pause behavior implemented; maintenance windows pending |
| DRAIN-07 | P1 | Pause, resume, and restart recovery | Persist resumable state across cancellation boundaries, browser loss, worker restart, and service restart. | IMPLEMENTED + SOFTWARE VERIFIED — durable manifests, UI/API pause/resume, stale-worker requeue, and manifest-preserving restart tests; Linux execution pending |
...
| DRAIN-09 | P1 | Verification modes | Implement FAST, ACCURATE/BLAKE3 where available, and PARANOID full-read verification with explicit evidence. | IN PROGRESS — FAST, SHA-256 ACCURATE, and two-pass PARANOID implemented; optional BLAKE3 optimization and Linux execution pending |
| DRAIN-10 | P1 | Finalize and namespace reconciliation | Remove/decommission only after verification and preserve stable ARR/share paths. | IMPLEMENTED + SOFTWARE VERIFIED — source deletion requires verified digest; read-only/retired transitions and stable namespace report implemented; Linux execution pending |
