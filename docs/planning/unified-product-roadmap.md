...
| AUTO-06 | P2 | Alert deduplication, routing, and suppression | Add windows, webhook/AppRise-compatible routing, runbook links, and flapping control. | IN PROGRESS — durable rule/entity deduplication, hysteresis, sustained windows, path-flap detection, bounded operator suppression, and signed generic-webhook routing are implemented; an AppRise-specific adapter and evidence-aware runbook links remain |
...
| AUTO-08 | P2 | Automation integration tests | Test token scope, schema stability, webhook replay/outage, Prometheus cardinality, alert lifecycle, and redaction. | IN PROGRESS — token scope/schema/redaction, Prometheus bounds, alert lifecycle, transaction-level webhook deduplication, signing, secret redaction, transient outage/retry, permanent rejection and real alert open/clear routing pass deterministic tests; a live external webhook receiver remains VALID-04 |
...
| VALID-04 | P2 | Backup and automation integration | Execute S3 outage/resume/checksum/restore and HA/webhook/metrics contract tests. | IN PROGRESS — Ubuntu 24.04 CI completed live MinIO target proof, durable upload, full remote SHA-256 verification, download, safe extraction and SQLite restore validation; deterministic webhook signing/outage/retry and HA/metrics contracts pass, while a live external webhook receiver remains pending |
