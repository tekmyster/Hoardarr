...
| LIFE-06 | P1 | Drain, verify, read-only, and retire states | Model DRAIN → VERIFY → READ-ONLY → RETIRE without permitting unsafe direct skips. | IN PROGRESS — states guarded; durable mover pending |
...
| LIFE-08 | P1 | Storage lifecycle API | Expose bounded authenticated group, disk, backend, lifecycle, and event contracts with Problem Details errors. | IN PROGRESS — safe pre-drain API verified |
| LIFE-09 | P1 | Storage Groups UI | Provide real loading, empty, error, group, backend, namespace, preferred-write, and lifecycle presentation. | IN PROGRESS — create/assign/activate/prefer UI verified |
| LIFE-10 | P1 | Lifecycle events and audit | Persist ordered state-change evidence, actor, reason, operation, and sanitized details. | IN PROGRESS — implemented transitions verified; operation linkage pending |
...
| LIFE-12 | P1 | Lifecycle end-to-end validation | Prove add, assign, activate, prefer, drain, verify, read-only, retire, and optional reuse/wipe behavior. | QUEUED |
...
| DRAIN-05 | P1 | Capacity and destination-health gates | Fail before and during movement when reserve, health, or destination identity becomes unsafe. | QUEUED |
| DRAIN-06 | P1 | ARR active-write awareness and windows | Coordinate imports/downloads/renames and configurable maintenance windows. | QUEUED |
| DRAIN-07 | P1 | Pause, resume, and restart recovery | Persist resumable state across cancellation boundaries, browser loss, worker restart, and service restart. | QUEUED |
| DRAIN-08 | P1 | Bandwidth and scheduling controls | Enforce bounded concurrency, bandwidth, IO priority, and operator scheduling. | QUEUED |
| DRAIN-09 | P1 | Verification modes | Implement FAST, ACCURATE/BLAKE3 where available, and PARANOID full-read verification with explicit evidence. | QUEUED |
| DRAIN-10 | P1 | Finalize and namespace reconciliation | Remove/decommission only after verification and preserve stable ARR/share paths. | QUEUED |
| DRAIN-11 | P1 | Drain reports and cost UX | Show files/bytes/errors, checkpoints, verification evidence, and measured/estimated CPU and IO cost. | QUEUED |
| DRAIN-12 | P1 | Drain failure matrix | Test full destination, disappearing source/destination, ARR conflict, checksum failure, pause/resume, and worker crash. | QUEUED |
