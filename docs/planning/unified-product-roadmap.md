...
| IMPORT-11 | P2 | Archive durable intake and report | Copy, checkpoint, verify, resolve collisions, and produce a final manifest/report. | SOFTWARE VERIFIED / SELECTED EXECUTION PENDING — the production foreign migration worker now checkpoints only reviewed selections, proves exact capacity before the first write, preserves relative paths, explicitly stops/reuses collisions, verifies content, supports pause/resume/restart, retains the source and records selection plus manifest totals in Activity; full-copy loop execution is verified and selected-folder Linux execution is included in the next hosted backend run |
| IMPORT-12 | P2 | Foreign/import integration tests | Exercise disposable filesystems, Unraid-like disks, malformed layouts, collisions, read errors, and restart recovery. | IN PROGRESS — deterministic executor tests cover signature drift, unexpected read-write state, guaranteed detach, detach failure, bounded inventory, symlink exclusion, API authorization/idempotency and durable worker completion; hosted Ubuntu run `32680220818` proves real ext4 inspection, Unraid data/parity classification and production foreign migration with read-only source retention, pause/resume, restart and stale-mount recovery, exact tree hashes, collision refusal, verified checkpoints, and detachment; injected source read-error execution remains pending |
