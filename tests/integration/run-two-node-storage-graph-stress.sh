...
remote 2222 'sudo sync; echo 3 | sudo tee /proc/sys/vm/drop_caches >/dev/null; sudo fio --name=mixed-read-write --filename=/srv/hoardarr/a/local/dataset/large.bin --rw=randrw --rwmixread=95 --bs=64k --size=8M --time_based=1 --runtime=12 --rate=1M,52k --direct=0 --iodepth=1 --invalidate=1 --output-format=json; sudo sync' >"${OUTPUT}/fio-mixed-read.json"
...
mixed_read_bytes = comparisons["mixed_read"]["fio"]["bytes"]
mixed_write_bytes = comparisons["mixed_write"]["fio"]["bytes"]
mixed_total_bytes = mixed_read_bytes + mixed_write_bytes
if mixed_write_bytes <= 0 or mixed_total_bytes <= 0 or mixed_read_bytes / mixed_total_bytes < 0.90:
    raise SystemExit(
        "mixed workload was not read-dominant: "
        f"read={mixed_read_bytes} write={mixed_write_bytes}"
    )
