...
type RedundancyAction = "add" | "remove" | "replace";
...
export function StorageRedundancyPanel() {
  ...
  const refresh = useCallback(async () => {
    try {
      setItems(await api.logicalStorage());
      setError(null);
    } catch (requestError) {
      setError(
  ...
  const open = (storage: LogicalStorageDocument, nextAction: RedundancyAction) => {
    setSelected(storage);
    ...
    {selected && <div className="wizard-overlay" role="presentation">
      <section className="wizard-dialog" role="dialog" aria-modal="true" aria-labelledby="redundancy-title">
        <header className="wizard-header">
          <div><small>ADVANCED STORAGE</small><h2 id="redundancy-title">{action === "add" ? "Add storage redundancy" : action === "replace" ? "Replace a controller path" : "Remove a storage path"}</h2></div>
        ...
        <div className="wizard-scroll">
          ...
          <Card title={selected.name} description={`${selected.mountpoint} · ${selected.filesystem_uuid ?? "Filesystem UUID not reported"}`}>
            {!plan && !operation && <>
              <Notice tone={action === "remove" ? "warning" : "info"} title={action === "add" ? "Your existing storage is preserved" : action === "replace" ? "The replacement is added first" : "Protection will be reduced"}>
                {action === "add"
        ...
        <footer className="wizard-footer">
          ...
          {!operation && (!plan
            ...
            : <button type="button" className="button button-primary" onClick={() => void apply()} disabled={busy}>{busy ? "Starting…" : action === "add" ? "Add redundancy" : action === "replace" ? "Replace path" : "Remove path"}</button>)}
        </footer>
      </section>
    </div>}
  </>;
