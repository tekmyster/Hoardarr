...
import type { StorageExpansionAssessment, StorageExpansionSelection } from "../types";
...
function capabilityState(configured: boolean, available: boolean | undefined): string {
  if (available === undefined) return configured ? "Detected" : "Not configured";
  if (configured && available) return "Configured and ready";
  if (configured) return "Configured · required software unavailable";
  return available ? "Software available" : "Not available";
}
...
export function StorageExpansionPanel({
  ...
  onPlan: (action: DriveAction, diskIds: string[], selection?: StorageExpansionSelection) => void;
  ...
  const diskNames = useMemo(() => new Map(
    [...(assessment?.available_disks ?? []), ...reservedDisks].flatMap((disk) => {
      const name = `${disk.vendor ?? ""} ${disk.model ?? "Disk"}`.trim();
      return [[disk.id, name], [disk.device_id ?? disk.stable_identity, name]];
    }),
  ), [assessment, reservedDisks]);

  const registryDiskId = (deviceId: string): string => {
    const disk = [...(assessment?.available_disks ?? []), ...reservedDisks].find((item) =>
      item.id === deviceId || item.device_id === deviceId || item.stable_identity === deviceId,
    );
    return disk?.id ?? deviceId;
  };
  ...
  return <Card title="Plan storage expansion" description="See safe choices for newly detected disks before changing storage.">
    ...
    {loading && !assessment ? <p role="status">Analyzing current Storage Groups and unassigned disks…</p> : assessment && assessment.available_disks.length === 0 && reservedDisks.length === 0 ? <div className="empty-state compact-empty"><h3>No unassigned disks detected</h3><p>Add or release a disk, then run a storage scan. Existing managed storage will not be changed.</p></div> : assessment && <>
      ...
      <div className="expansion-capabilities" aria-label="Detected expansion capabilities"><span>mergerFS: <strong>{capabilityState(assessment.detected_capabilities.mergerfs, assessment.tool_availability?.mergerfs)}</strong></span><span>SnapRAID: <strong>{capabilityState(assessment.detected_capabilities.snapraid, assessment.tool_availability?.snapraid)}</strong></span><span>ZFS: <strong>{capabilityState(assessment.detected_capabilities.zfs, assessment.tool_availability?.zfs)}</strong></span><span>Linux RAID: <strong>{capabilityState(assessment.detected_capabilities.linux_md ?? false, assessment.tool_availability?.linux_md)}</strong></span></div>
      ...
      {assessment.candidates.length === 0 ? <Notice tone="warning" title="No safe expansion plan yet">The detected disks need health, identity, or existing-data review before Hoardarr can recommend a change.</Notice> : <div className="expansion-candidate-list">{assessment.candidates.map((candidate) => <article key={candidate.id} className={candidate.recommended ? "recommended" : ""} aria-label={candidate.title}>
        ...
        <div className="button-row"><button className="button button-primary" type="button" onClick={() => onPlan(setupAction(candidate.setup_mode), candidate.disk_ids, { candidate_id: candidate.id, kind: candidate.kind, storage_group_id: candidate.storage_group_id, hardware_snapshot_sha256: assessment.hardware_snapshot_sha256, disk_ids: candidate.disk_ids, target: candidate.target, configuration: candidate.configuration })}>{candidate.setup_mode === "import" ? "Review existing storage" : "Customize this plan"}</button>{candidate.disk_ids.length === 1 && <button className="button button-secondary" type="button" disabled={reservationBusy === registryDiskId(candidate.disk_ids[0])} onClick={() => void changeReservation(registryDiskId(candidate.disk_ids[0]), "reserve")}>{reservationBusy === registryDiskId(candidate.disk_ids[0]) ? "Reserving…" : "Reserve for later"}</button>}</div>
      </article>)}</div>}
