...
export function StorageExpansionPanel({
  ...
  return <Card title="Plan storage expansion" description="See safe choices for newly detected disks before changing storage.">
    ...
    {loading && !assessment ? <p role="status">Analyzing current Storage Groups and unassigned disks…</p> : assessment && assessment.available_disks.length === 0 ? <div className="empty-state compact-empty"><h3>No unassigned disks detected</h3><p>Add or release a disk, then run a storage scan. Existing managed storage will not be changed.</p></div> : assessment && <>
      ...
      {assessment.candidates.length === 0 ? <Notice tone="warning" title="No safe expansion plan yet">The detected disks need health, identity, or existing-data review before Hoardarr can recommend a change.</Notice> : <div className="expansion-candidate-list">{assessment.candidates.map((candidate) => <article key={candidate.id} className={candidate.recommended ? "recommended" : ""} aria-label={candidate.title}>
        ...
        <button className="button button-primary" type="button" onClick={() => onPlan(setupAction(candidate.setup_mode), candidate.disk_ids)}>{candidate.setup_mode === "import" ? "Review existing storage" : "Customize this plan"}</button>
      </article>)}</div>}
