...
export function AnalyticsPage({ context }: { context?: MetricHistoryContext | null }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setSelectedEntity((value) => {
        ...
        return resolveMetricHistoryEntity(entityItems, context)?.id ?? entityItems[0]?.id ?? "";
      });
      setError(null);
    } catch (reason) {
  ...
  }, [applicableDefinitions, context, contextKey, definitions, entities, selectedEntityRecord, selectedMetric]);
  ...
  useEffect(() => {
    ...
    if (!selectedEntity || !selectedMetric || !definitionApplies) {
      setHistory(null);
  ...
  }, [historySettings?.history.maximum_graph_points, rangeHours, selectedDefinition, selectedEntity, selectedEntityType, selectedMetric]);
  ...
  useEffect(() => {
    ...
    setTopItems([]);
    setAnomalies([]);
    if (!entitlements || !selectedEntity || !selectedMetric || !selectedDefinition?.entity_types.includes(selectedEntityType ?? "")) return;
    const controller = new AbortController();
    ...
    return () => controller.abort();
  }, [capabilityKey, selectedDefinition, selectedEntity, selectedEntityType, selectedMetric]);
  ...
  const contextualSelectionUnavailable = Boolean(context && entities.length && !contextualEntity);
  ...
  return <section className="analytics-page" aria-label="Storage analytics">
    ...
    {contextualSelectionUnavailable && <Notice tone="warning" title="Storage item is not currently reported">The requested {context?.entityType.replaceAll("_", " ")} identity could not be matched safely. Choose a reported item below; Hoardarr did not substitute a similarly named device.</Notice>}
    ...
    <section className="analytics-section" aria-busy={historyLoading}><div className="section-title-row"><h3>Persistent history</h3>{history && <span>{history.points.length} points</span>}</div>{historyLoading ? <p className="empty-state">Loading stored telemetry…</p> : historyError ? <Notice tone="danger" title="History unavailable">{historyError}</Notice> : history?.points.length ? <>{categoricalHistory ? <CategoricalMetricHistory history={history} label={selectedDefinition?.name ?? selectedMetric} /> : <NumericMetricHistory history={history} label={selectedDefinition?.name ?? selectedMetric} graphType={graphType} unit={selectedDefinition?.unit ?? history.unit} />}<MetricHistoryDiagnostics history={history} definition={selectedDefinition} settings={historySettings} /></> : <div className="empty-state"><p>No stored readings are available for this selection.</p><small>Missing history is not shown as zero or idle.</small></div>}{selectedDefinition && <MetricDefinitionHelp metric={selectedDefinition} />}</section>
