    ...
    async function refreshDetails(): Promise<void> {
      try {
        const [foundEvents, foundProgress] = await Promise.all([
          ...
          operation.kind === "storage.apply" ? api.storageOperationProgress(operation.id).catch(() => null) : Promise.resolve(null),
        ]);
        if (!cancelled) {
