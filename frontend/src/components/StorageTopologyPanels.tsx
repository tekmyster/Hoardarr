...
function PhysicalNode({ node }: { node: StorageTopologyNode }) {
  return <article className={`topology-logical-node topology-${node.kind}`}>
    <header><div><span>{node.kind === "phy" ? "SAS PHY" : node.kind === "sas_host" ? "SAS host" : node.kind}</span><strong>{node.label}</strong>{node.address && <code>{node.address}</code>}</div><StatusBadge status={node.status ?? "detected"} /></header>
    {node.kind === "phy" && <dl>
