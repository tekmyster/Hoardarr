...
export function HealthPage({ onOpenHistory }: { onOpenHistory?: (context: MetricHistoryContext) => void } = {}) {
    ...
    {busy && !snapshot && !inventory ? <Spinner label="Loading health" /> : <>
      ...
      <Card title="Drives" description={snapshot ? `Last scan ${new Date(snapshot.captured_at).toLocaleString()}` : "No hardware scan is available."}>
        {!snapshot ? <div className="empty-state compact-empty"><h3>No hardware scan</h3><p>Run storage discovery to load drive health.</p></div> : !drives.length ? <div className="empty-state compact-empty"><h3>No data drives detected</h3><p>The system drive is intentionally excluded.</p></div> : <div className="table-scroll"><table className="data-table"><thead><tr><th>Drive</th><th>Health</th><th>Temperature</th><th>Power-on hours</th><th>Lifetime writes</th><th>Endurance remaining</th><th>Latest test</th>{onOpenHistory && <th>History</th>}</tr></thead><tbody>{drives.map((drive) => {
          ...
          return <tr key={drive.id}><td><strong>{drive.vendor} {drive.model}</strong><small className="cell-detail">{drive.serial}</small></td><td><StatusBadge status={drive.healthStatus === "unknown" ? "Not reported" : drive.healthStatus} /></td><td>{temperature?.available && typeof temperature.value === "number" ? `${temperature.value} °C` : "Not reported"}</td><td>{power?.available && typeof power.value === "number" ? numberLabel(power.value, " h") : "Not reported"}</td><td>{bytesLabel(driveEndurance?.lifetime_writes_bytes ?? null)}</td><td>{numberLabel(driveEndurance?.remaining_percent ?? null, "%")}</td><td>{latestTest ? <><StatusBadge status={latestTest.status} /><small className="cell-detail">{latestTest.label}</small></> : "Not reported"}</td>{onOpenHistory && <td><button type="button" className="button button-compact" onClick={() => onOpenHistory({ entityType: "drive", stableId: drive.id, displayName: `${drive.vendor} ${drive.model}`.trim(), metricId: "drive.temperature", sourceSurface: "health" })}>Graph</button></td>}</tr>;
        })}</tbody></table></div>}
      </Card>
      <Card title="Controllers and enclosures" description="Health reported by installed controller and enclosure tools.">
        {!inventory?.controllers?.items.length && !inventory?.enclosures?.items.length ? <div className="empty-state compact-empty"><h3>Not reported</h3><p>No supported controller or enclosure health provider returned data.</p></div> : <div className="table-scroll"><table className="data-table"><thead><tr><th>Hardware</th><th>Provider</th><th>Health</th><th>Temperature</th><th>Cooling</th><th>Power / voltage</th>{onOpenHistory && <th>History</th>}</tr></thead><tbody>{inventory.controllers.items.map((controller) => <tr key={`${controller.provider}:${controller.id}`}><td><strong>{controller.model}</strong><small className="cell-detail">{controller.serial ?? "Not reported"}</small></td><td>{controller.provider}</td><td><StatusBadge status={controller.health} /></td><td>Not reported</td><td>Not reported</td><td>Not reported</td>{onOpenHistory && <td><button type="button" className="button button-compact" onClick={() => onOpenHistory({ entityType: "controller", stableId: `controller:${controller.provider}:${controller.serial ?? controller.id}`, displayName: controller.model, metricId: "health.overall", sourceSurface: "health" })}>Graph</button></td>}</tr>)}{inventory.enclosures.items.map((enclosure) => <tr key={`${enclosure.provider}:${enclosure.path}`}><td><strong>{enclosure.descriptor}</strong><small className="cell-detail">{enclosure.id}</small></td><td>{enclosure.provider}</td><td><StatusBadge status={enclosure.health} /></td><td>{typeof enclosure.temperature_c === "number" ? `${enclosure.temperature_c} °C` : "Not reported"}</td><td>{typeof enclosure.fan_rpm === "number" ? `${enclosure.fan_rpm.toLocaleString()} RPM · ${enclosure.fan_count} reported` : "Not reported"}</td><td>{Array.isArray(enclosure.power_supplies) ? `${enclosure.power_supplies.join(", ")} · ${Array.isArray(enclosure.voltages) ? enclosure.voltages.map((value) => `${value} V`).join(", ") : "voltage not reported"}` : "Not reported"}</td>{onOpenHistory && <td><button type="button" className="button button-compact" onClick={() => onOpenHistory({ entityType: "enclosure", stableId: `ses:${enclosure.id}`, displayName: enclosure.descriptor, metricId: "enclosure.temperature", sourceSurface: "health" })}>Graph</button></td>}</tr>)}</tbody></table></div>}
      </Card>
      <Card title="Pools" description="Live pool state and capacity.">
        {!inventory?.pools.items.length ? <div className="empty-state compact-empty"><h3>No pools configured</h3><p>Individual drives are listed above.</p></div> : <div className="table-scroll"><table className="data-table"><thead><tr><th>Pool</th><th>Type</th><th>Status</th><th>Maintenance</th><th>Progress</th><th>Used</th><th>Free</th><th>Members</th>{onOpenHistory && <th>History</th>}</tr></thead><tbody>{inventory.pools.items.map((pool) => <tr key={pool.id}><td><strong>{pool.name}</strong></td><td>{pool.type}</td><td><StatusBadge status={pool.status} /></td><td>{pool.maintenance ?? "Not reported"}</td><td>{typeof pool.progress_percent === "number" ? `${pool.progress_percent}%` : "Not reported"}</td><td>{bytesLabel(pool.used_bytes)}</td><td>{bytesLabel(pool.free_bytes)}</td><td>{numberLabel(pool.members)}</td>{onOpenHistory && <td><button type="button" className="button button-compact" onClick={() => onOpenHistory({ entityType: "pool", stableId: pool.id, displayName: pool.name, metricId: "health.overall", sourceSurface: "health" })}>Graph</button></td>}</tr>)}</tbody></table></div>}
      </Card>
