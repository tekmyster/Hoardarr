  ...
  return <div className="health-page">
    ...
    {busy && !snapshot && !inventory ? <Spinner label="Loading health" /> : <>
      ...
      <Card title="Controllers and enclosures" description="Health reported by installed controller and enclosure tools.">
        {!inventory?.controllers?.items.length ? <div className="empty-state compact-empty"><h3>Not reported</h3><p>No supported controller health provider returned data.</p></div> : <div className="table-scroll"><table className="data-table"><thead><tr><th>Controller</th><th>Provider</th><th>Serial</th><th>Health</th></tr></thead><tbody>{inventory.controllers.items.map((controller) => <tr key={`${controller.provider}:${controller.id}`}><td>{controller.model}</td><td>{controller.provider}</td><td>{controller.serial ?? "Not reported"}</td><td><StatusBadge status={controller.health} /></td></tr>)}</tbody></table></div>}
      </Card>
