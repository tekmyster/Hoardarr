              ...
              <div className="drive-action-dropdown" role="menu" aria-label={`Choose what to do with ${drive.path}`}>
                {DRIVE_ACTIONS.map((action) => <button
                  ...
                  onClick={(event) => {
                    ...
                    if (action.id === "wipe") {
                      setMaintenanceAction("wipe");
                      setWipeMethod("metadata_clear");
                      setMaintenanceDrive(drive);
                    }
      ...
      <div className="wizard-scroll">
        ...
        <Card title={`${maintenanceDrive.vendor} ${maintenanceDrive.model}`} description={`${maintenanceDrive.path} · ${maintenanceDrive.serial} · ${humanCapacity(maintenanceDrive.capacityBytes)}`}>
          {!maintenanceOperation && <div className="form-grid">
            ...
            {maintenanceAction === "wipe" ? <><label>Method<select value={wipeMethod} onChange={(event) => { setWipeMethod(event.target.value as typeof wipeMethod); setMaintenancePlan(null); }}><option value="metadata_clear">Clear filesystem/signature metadata only</option><option value="hdd_overwrite" disabled={maintenanceDrive.rotational !== true}>One-pass HDD overwrite{maintenanceDrive.rotational !== true ? " — unavailable" : ""}</option><option value="ata_secure_erase" disabled={!maintenanceDrive.maintenanceCapabilities?.ataSecureErase}>ATA secure erase{!maintenanceDrive.maintenanceCapabilities?.ataSecureErase ? " — not reported" : ""}</option><option value="nvme_sanitize" disabled={!maintenanceDrive.maintenanceCapabilities?.nvmeBlockErase}>NVMe block erase{!maintenanceDrive.maintenanceCapabilities?.nvmeBlockErase ? " — not reported" : ""}</option><option value="nvme_crypto_erase" disabled={!maintenanceDrive.maintenanceCapabilities?.nvmeCryptoErase}>NVMe cryptographic erase{!maintenanceDrive.maintenanceCapabilities?.nvmeCryptoErase ? " — not reported" : ""}</option><option value="scsi_sanitize" disabled={!maintenanceDrive.maintenanceCapabilities?.scsiBlockErase}>SCSI block erase{!maintenanceDrive.maintenanceCapabilities?.scsiBlockErase ? " — not reported" : ""}</option><option value="scsi_crypto_erase" disabled={!maintenanceDrive.maintenanceCapabilities?.scsiCryptoErase}>SCSI cryptographic erase{!maintenanceDrive.maintenanceCapabilities?.scsiCryptoErase ? " — not reported" : ""}</option></select></label><Notice tone="warning" title={wipeMethod === "metadata_clear" ? "Metadata clear is not media sanitization" : "This erases user data"}>{wipeMethod === "metadata_clear" ? "This removes filesystem and partition signatures only. Existing bytes may remain recoverable." : wipeMethod === "hdd_overwrite" ? "Hoardarr permits overwrite only for a drive reported as rotational. Multiple passes are not presented as stronger modern sanitization." : `Support was reported by ${maintenanceDrive.maintenanceCapabilities?.source ?? "the latest hardware scan"}. Capability and identity will be checked again before execution.`}</Notice></> : <label>Target sector size<select value={targetSector} onChange={(event) => { setTargetSector(Number(event.target.value) as 512 | 4096); setMaintenancePlan(null); }}><option value={512}>512 bytes</option><option value={4096}>4096 bytes</option></select></label>}
          </div>}
          ...
          {maintenanceOperation && <><div className="storage-operation-heading"><StatusBadge status={maintenanceOperation.status.replace("_", " ")} /><strong>{maintenanceProgress?.percent ?? 0}%</strong></div><div className="operation-progress-track" role="progressbar" aria-label="Drive maintenance progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={maintenanceProgress?.percent ?? 0}><span style={{ width: `${maintenanceProgress?.percent ?? 0}%` }} /></div><StorageProgressDetails progress={maintenanceProgress} />{(maintenanceOperation.result?.sanitization_report || maintenanceProgress?.sanitization_report) && <details open><summary>Sanitization report</summary><pre>{JSON.stringify(maintenanceOperation.result?.sanitization_report ?? maintenanceProgress?.sanitization_report, null, 2)}</pre></details>}{maintenanceOperation.error && <Notice tone="danger" title="Drive maintenance failed">{maintenanceOperation.error.message ?? maintenanceOperation.error.detail ?? "The operation did not finish."}</Notice>}</>}
        </Card>
      </div>
